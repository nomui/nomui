define([], function () {
  return {
    ...
    file: 'view',
    description:
      '当 `content` 配置的值为字符串时，代表一个指向某个 js 模块（视图）的 url，当模态框的内容比较复杂或者可多处复用时，将模态框内容分离到独立的 js 模块是一个好的代码实践，这样模态框的内容低耦合高内聚，方便维护。独立出来的 js 模块返回一个函数，函数的参数是一个对象`{ drawer, args }` 其中 `drawer` 是抽屉自身的实例，`args` 是模态框调用方传递的参数（通过 args prop）。',
    demo: function () {
      return {
        component: 'Flex',
        gap: 'small',
